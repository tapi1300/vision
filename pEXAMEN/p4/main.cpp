...
// Trackbar values
int max_selector = 2, max_Canny = 255, max_accumulator = 300, max_radius = 50, max_aspect_ratio = 4, max_vert = 1;
int selector_val = 0, Canny_val = 100, accumulator_val = 200, radius_val = 30, aspect_ratio_val = 1, vert_val = 0;
...
string vert_trackbar = "Vertical 0 - Horizontal 1:";
...
Mat get_Hough(Mat canny)
{
	/*This function receives a Mat with the image already filtered with Canny, and returns
		the original image with the hough lines (vertical or horizontal) and circles drawn.
	*/
    ...
    for( size_t i = 0; i < lines.size(); i++ ) {
		...
		if((vert_val == 0 && pt1.y != pt2.y) || (vert_val == 1 && pt1.y == pt2.y))
		{
			// Si se quieren mostrar las lineas verticales y la "y" de los puntos son distintas
			//	o si se quiere mostrar las lineas horizontales y la x de ambos puntos son iguales,
			//	se muestra la linea.
...
int main(int argc, char** argv)
{
	...
	createTrackbar(vert_trackbar, main_name, &vert_val, max_vert, ChangeVal, (void *)5);
	waitKey();
